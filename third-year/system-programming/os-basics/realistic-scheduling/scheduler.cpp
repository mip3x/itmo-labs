...
static std::queue<int> *threads_queue;
static int current_thread_id;
static int _timeslice;

// квант времени, выделенный потоку на выполнение
static int ticks;
...
void switch_to_next_thread()
{
    ...
    if (threads_queue->size() > 0)
    {
        current_thread_id = threads_queue->front();
        threads_queue->pop();
    }
    else
    {
...
{
    threads_queue = new std::queue<int>();
...
void timer_tick()
{
    ...
    if (--ticks == 0)
    {
        threads_queue->push(current_thread_id);
        switch_to_next_thread();
    }
}

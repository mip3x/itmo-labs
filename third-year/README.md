# third-year
## Лабораторные работы III-го курса
### Список дисциплин:
- [ОС (Операционные системы)](https://github.com/mip3x/os-course)
- [СИИ (Системы искусственного интеллекта)](./aisystems)
- [СПО (Системное программное обеспечение)](./system-programming)
- [ФП (Функциональное программирование)](./functional-programming)
- [ВСС (Встроенные системы)](./embedded-systems)
- [ИС (Информационные системы)](./information-systems)
- [СВВ (Системы ввода-вывода)](./io-systems)
- [ТПО (Тестирование программного обеспечения)](./software-testing)
- [РСХД (Распределённые системы хранения данных)](./dbms)
- [КС (Компьютерные сети)](./networks)

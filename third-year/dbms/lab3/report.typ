...
В ходе лабораторной работы была развернута отказоустойчивая конфигурация PostgreSQL в сети виртуальных машин с применением репликации журналов транзакций между основным и резервным серверами. Это обеспечило синхронизацию данных между двумя узлами кластера. В процессе работы была выполнена имитация отказа основного сервера и успешно проведено переключение на резервный узел без потери работоспособности системы. После восстановления основного сервера выполнена повторная синхронизация данных и возврат его в состав кластера.

Практическая работа продемонстрировала эффективность механизма репликации журналов для повышения отказоустойчивости и снижения риска потери данных при сбоях. Полученные результаты подтвердили возможность применения подобных решений для обеспечения надежной работы PostgreSQL-кластеров в реальных условиях эксплуатации.

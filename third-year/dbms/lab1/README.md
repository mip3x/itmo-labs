...
Конфигурация порта. Откроем файл `postgresql.conf`:
...
Перейдём к настройке параметров сервера БД. Сделаем это через `ALTER SYSTEM` из `sql`. В таком случае, параметры будут записаны в `postgresql.auto.conf`, который имеет оолее слабый приоритет в сравнении с `postgresql.conf`, но подходит для конфигурации дополнительных параметров:

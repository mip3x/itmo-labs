...
Для генерации ARP-трафика использовались команды:

```text
sudo arping -c 1 -I wlp1s0 192.168.0.1
dig se.ifmo.ru +short
sudo arping -c 1 -I wlp1s0 77.234.196.4
```

Локальный IP-адрес компьютера: `192.168.0.17`. MAC-адрес сетевой карты: `74:4c:a1:c3:39:7b`. IP-адрес шлюза: `192.168.0.1`, MAC-адрес шлюза: `88:d2:74:ac:95:06`. Домен `se.ifmo.ru` разрешился в IP-адрес `77.234.196.4`.

#figure(
  image("arp_full_screen.png", width: 100%),
  caption: [ARP-запросы и ответ в локальной сети]
)

#figure(
  image("arp_mac_addrs.png", width: 100%),
  caption: [MAC-адреса компьютера и роутера в ARP-пакете]
)

== Ответы на вопросы

1. Какие MAC-адреса присутствуют в захваченных пакетах ARP-протокола? Что означают эти адреса?

В ARP-пакетах присутствуют MAC-адрес компьютера `74:4c:a1:c3:39:7b`, MAC-адрес шлюза `88:d2:74:ac:95:06` и широковещательный адрес `ff:ff:ff:ff:ff:ff`. Первый адрес идентифицирует сетевую карту ноутбука, второй — роутер в локальной сети, третий используется для ARP-запроса ко всем устройствам локальной сети.

2. Какие MAC-адреса присутствуют в захваченных HTTP-пакетах и что означают эти адреса?

При обращении к удалённому сайту в Ethernet-кадре используются MAC-адрес компьютера и MAC-адрес шлюза. MAC-адрес удалённого сайта в локальном кадре не появляется, так как сайт находится вне локальной сети. IP-адрес назначения остаётся адресом сайта `77.234.196.4`, а MAC-адрес назначения — адрес ближайшего шлюза `88:d2:74:ac:95:06`.

3. Для чего ARP-запрос содержит IP-адрес источника?

IP-адрес источника нужен, чтобы устройство-получатель знало, кому отправить ARP-ответ. Также получатель может сохранить соответствие `IP -> MAC` отправителя в своём ARP-кэше.

ARP-запрос к адресу `77.234.196.4` не получил ответа, потому что этот адрес не находится в локальной сети `192.168.0.0/24`. Для доступа к удалённому сайту ARP используется только для определения MAC-адреса шлюза.

= Анализ DNS-трафика утилиты nslookup

Для анализа DNS-трафика использовались команды:

```text
nslookup se.ifmo.ru
nslookup -type=NS se.ifmo.ru
```

DNS-сервер, к которому обращалась система: `78.37.77.77`. Домен `se.ifmo.ru` был разрешён в IP-адрес `77.234.196.4`.
...
Non-authoritative answer:
se.ifmo.ru      nameserver = tdcgate.tune-it.ru.
se.ifmo.ru      nameserver = cldgate.tune-it.ru.
se.ifmo.ru      nameserver = sunrise.cs.ifmo.ru.
se.ifmo.ru      nameserver = tdcgate2.tune-it.ru.
se.ifmo.ru      nameserver = orion.MATPOCKuH.ru.

Authoritative answers can be found from:
```

#figure(
  image("nslookup_full_scheme.png", width: 100%),
  caption: [DNS-запрос nslookup для se.ifmo.ru]
)

#figure(
  image("nslookup_answers.png", width: 100%),
  caption: [Поле Answers в DNS-ответе]
)

#figure(
  image("nslookup_ns_full_scheme.png", width: 100%),
  caption: [DNS-запрос nslookup с типом NS]
)
...
В первом случае выполняется обычный DNS-запрос типа `A`, который возвращает IPv4-адрес домена. Во втором случае выполняется запрос типа `NS`, который возвращает имена DNS-серверов, обслуживающих домен.

2. Что содержится в поле `Answers` DNS-ответа?

В ответе на обычный запрос поле `Answers` содержит A-запись: `se.ifmo.ru -> 77.234.196.4`. В ответе на запрос `NS` поле `Answers` содержит имена серверов имён для домена.

3. Каковы имена серверов, возвращающих авторитативный отклик?

Авторитативный ответ напрямую получен не был: `nslookup` вывел `Non-authoritative answer`. Это значит, что ответ был получен от рекурсивного DNS-сервера `78.37.77.77`, а не напрямую от сервера, обслуживающего зону.

Однако в ответе типа `NS` были получены имена серверов, у которых можно получить авторитативный ответ для зоны `se.ifmo.ru`:
...
В ходе лабораторной работы был проанализирован сетевой трафик протоколов ICMP, IPv4, ARP и DNS в программе Wireshark.

При анализе `ping` было показано, что ICMP передаётся внутри IPv4-пакета как полезная нагрузка. Фрагментация IPv4 определяется по полям `More fragments`, `Fragment Offset` и `Identification`.

При анализе `traceroute` было установлено, что маршрут определяется за счёт постепенного увеличения поля `TTL`: промежуточные маршрутизаторы возвращают ICMP-ошибки, а конечный узел возвращает ответ.

При анализе `ARP` было показано, что MAC-адрес определяется только в пределах локальной сети. Для обращения к удалённому сайту компьютер узнаёт не MAC-адрес сайта, а MAC-адрес шлюза.

При анализе `nslookup` были рассмотрены DNS-запросы типов `A` и `NS`. Запрос `A` возвращает IPv4-адрес домена, а запрос `NS` возвращает имена DNS-серверов, обслуживающих доменную зону.

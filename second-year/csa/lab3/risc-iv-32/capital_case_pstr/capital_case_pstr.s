...
success:
    lui     t2, %hi(pascal_str_len)        ; t2 <-- pascal_str_len:addr
    addi    t2, t2, %lo(pascal_str_len)
    sb      t1, 0(t2)                      ; pascal_str_len <-- index (message size)
    lui     a0, 0
...
output_buf:
    ...
    lui     a2, %hi(pascal_str_len)
    addi    a2, a2, %lo(pascal_str_len)
